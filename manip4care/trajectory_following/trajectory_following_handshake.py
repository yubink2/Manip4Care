...
from manip4care.envs.agents.pybullet_ur5.robot import UR5Robotiq85
from manip4care.envs.agents.humanoid_supine import Humanoid
from manip4care.envs.agents.humanoid_sitting import HumanoidSeated

class TrajectoryFollower_H_Handshake():
    def __init__(self, eef_to_cp, elbow_joint_to_cp,
        ...
        # initialize T_elbow_to_cp (constant)
        self.elbow_joint_to_cp = elbow_joint_to_cp
        self.cp_to_elbow_joint = self.bc_second.invertTransform(self.elbow_joint_to_cp[0], self.elbow_joint_to_cp[1])
    ...
    def clamp_human_joints(self, q_R):
        ...
        world_to_elbow_joint = self.bc_second.multiplyTransforms(world_to_cp[0], world_to_cp[1],
                                                        self.cp_to_elbow_joint[0], self.cp_to_elbow_joint[1])

        # IK -> get human joint angles
        q_H = self.bc_second.calculateInverseKinematics(self.humanoid._humanoid, self.elbow, 
                                                        targetPosition=world_to_elbow_joint[0], targetOrientation=world_to_elbow_joint[1],
        ...
        world_to_elbow_joint = self.bc_second.getLinkState(self.humanoid._humanoid, self.elbow)[4:6]
        world_to_cp = self.bc_second.multiplyTransforms(world_to_elbow_joint[0], world_to_elbow_joint[1],

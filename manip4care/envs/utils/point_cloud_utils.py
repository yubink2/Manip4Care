...
def get_humanoid_point_cloud(body_id, link_id_to_separate=None, client_id=0, resolution=8, extra_length=0, extra_radius=0):
            ...
            for data in mesh_data:
                if data[2] in [p.GEOM_BOX, p.GEOM_SPHERE, p.GEOM_CAPSULE, p.GEOM_CYLINDER, p.GEOM_MESH]:
                    ...
                    if data[2] == p.GEOM_BOX:
                        ...
                        vertices = generate_box_vertices(half_extents, position, orientation, resolution=resolution, client_id=client_id)
                    elif data[2] == p.GEOM_SPHERE:
                        radius = data[3][0]
                        vertices = generate_sphere_vertices(radius, position, orientation, resolution=resolution, client_id=client_id)
                    elif data[2] == p.GEOM_CAPSULE:
                        height = data[3][0] + extra_length
                        radius = data[3][1] + extra_radius
                        vertices = generate_capsule_vertices(radius, height, position, orientation, resolution=resolution, client_id=client_id)
                    elif data[2] == p.GEOM_CYLINDER:
                        height = data[3][0]
                        radius = data[3][1]
                        vertices = generate_cylinder_vertices(radius, height, position, orientation, resolution=resolution, client_id=client_id)
...
def get_point_cloud_from_collision_shapes(body_id, box_shape=None, client_id=0, resolution=8, num_joints=None):
    point_cloud = []
                    ...
                    if data[2] in [p.GEOM_MESH, p.GEOM_BOX, p.GEOM_SPHERE, p.GEOM_CYLINDER, p.GEOM_CAPSULE]:
                        ...
                        if data[2] == p.GEOM_MESH:
                            ...
                            if box_shape is not None:
                                vertices = generate_box_vertices(box_shape, position, orientation, client_id=client_id, resolution=resolution)
                            else:
                                vertices = generate_box_vertices(half_extents, position, orientation, client_id=client_id, resolution=resolution)
                        elif data[2] == p.GEOM_SPHERE:
                            radius = data[3][0]
                            vertices = generate_sphere_vertices(radius, position, orientation, client_id=client_id, resolution=resolution)
                        elif data[2] == p.GEOM_CYLINDER:
                            height = data[3][0]
                            radius = data[3][1]
                            vertices = generate_cylinder_vertices(radius, height, position, orientation, client_id=client_id, resolution=resolution)
                        elif data[2] == p.GEOM_CAPSULE:
                            height = data[3][0]
                            radius = data[3][1]
                            vertices = generate_capsule_vertices(radius, height, position, orientation, client_id=client_id, resolution=resolution)
...
def get_point_cloud_from_collision_shapes_specific_link(body_id, link_id, resolution=15, 
                                                        scale_radius=None, scale_height=None, skip_hemispherical=False, client_id=0):
    point_cloud = []
    collision_shapes = p.getCollisionShapeData(body_id, link_id, physicsClientId=client_id)

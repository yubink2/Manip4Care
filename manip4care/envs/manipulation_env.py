...
# mppi planner (ramp)
from manip4care.mppi_planning.trajectory_planning import TrajectoryPlanner
from manip4care.trajectory_following.trajectory_following import TrajectoryFollower
from manip4care.mppi_planning.mppi_human_handshake import MPPI_H_Handshake
...
# urdf paths
robot_urdf_location = 'manip4care/envs/agents/pybullet_ur5/urdf/ur5_robotiq_85.urdf'
scene_urdf_location = 'manip4care/resources/environment/environment.urdf'
control_points_location = 'manip4care/resources/ur5_control_points/T_control_points.json'
...
class ManipulationEnv(BaseEnv):
    ...
    def reset(self, wiping=False):
        ...
        if self.robot_w is not None:
            ...
            robot_w_obstacles = [self.bed_id, self.humanoid._humanoid, self.cube_m_id]
            self.robot_w_in_collision = get_collision_fn(self.robot_w.id, self.robot_w.arm_controllable_joints, obstacles=robot_w_obstacles,
    ...
    def init_tool(self):
        ...
        for j in self.human_arm:
            for tj in list(range(self.bc.getNumJoints(self.tool, physicsClientId=self.bc._client))) + [-1]:
                self.bc.setCollisionFilterPair(self.humanoid._humanoid, self.tool, j, tj, False, physicsClientId=self.bc._client)

        # disable collisions between the robot (wiping) and human arm
        for j in self.human_arm:
            for tj in list(range(self.bc.getNumJoints(self.robot_w.id, physicsClientId=self.bc._client))):
                self.bc.setCollisionFilterPair(self.humanoid._humanoid, self.robot_w.id, j, tj, False, physicsClientId=self.bc._client)

        # disable collisions between robot (manip) and second robot (wiping)
        for j in list(range(self.bc.getNumJoints(self.robot_m.id, physicsClientId=self.bc._client))):
            for tj in list(range(self.bc.getNumJoints(self.robot_w.id, physicsClientId=self.bc._client))):
                self.bc.setCollisionFilterPair(self.robot_m.id, self.robot_w.id, j, tj, False, physicsClientId=self.bc._client)
    ...
    def init_traj_planner(self, world_to_robot_base, clamp_by_human, q_H_init=None, q_R_init=None):
        JOINT_LIMITS = [
            np.array(self.robot_m.arm_lower_limits), 
            np.array(self.robot_m.arm_upper_limits)
        ]
        ...
        if clamp_by_human:
            ...
            mppi_H_handshake = MPPI_H_Handshake(self.eef_to_cp, self.elbow_joint_to_cp, 
                                                self.robot_m_base_pose, self.human_base_pose,
    ...
    def init_mppi_planner(self, trajectory_planner, current_joint_angles, target_joint_angles, clamp_by_human, init_traj=[]):
        # MPPI parameters
        N_JOINTS = len(self.robot_m.arm_controllable_joints)
        if clamp_by_human:
            mppi_control_limits = [
    ...
    def attach_human_arm_to_eef(self, attach_to_gripper=False, arm_pcd=None, trajectory_planner=None):
        ...
        eef_to_elbow = self.bc.multiplyTransforms(eef_to_world[0], eef_to_world[1],
                                                  world_to_elbow[0], world_to_elbow[1])

        if self.elbow_to_cp is None or self.eef_to_cp is None:
            raise ValueError('elbow_to_cp or eef_to_cp not initialized.')
        
        if self.human_cid is None:
            self.human_cid = self.bc.createConstraint(parentBodyUniqueId=self.robot_m.id,
                                                      parentLinkIndex=self.robot_m.eef_id,
                                                      childBodyUniqueId=self.humanoid._humanoid,
                                                      childLinkIndex=self.elbow,
                                                      jointType=p.JOINT_FIXED,
                                                      jointAxis=(0, 0, 0),
                                                      parentFramePosition=eef_to_elbow[0],
                                                      parentFrameOrientation=eef_to_elbow[1],
        ...
        if attach_to_gripper:
            ...
            trajectory_planner.attach_to_gripper(object_type="pcd", object_geometry=arm_pcd,
    ...
    def attach_tool(self, attach_to_gripper=False, tool_pcd=None, trajectory_planner=None):
        ...
        self.tool_cid = self.bc.createConstraint(parentBodyUniqueId=self.robot_w.id,
    ...
    def get_obj_base_pose(self, obj):
        if obj == "bed":
            ...
            return self.bc.getBasePositionAndOrientation(self.robot_m.id)
        elif obj == "robot_w":
            return self.bc.getBasePositionAndOrientation(self.robot_w.id)
        elif obj == "humanoid":
            return self.bc.getBasePositionAndOrientation(self.humanoid._humanoid)
        elif obj == "cube_m":
            return self.bc.getBasePositionAndOrientation(self.cube_m_id)
        elif obj == "cube_w":
            return self.bc.getBasePositionAndOrientation(self.cube_w_id)
        else:

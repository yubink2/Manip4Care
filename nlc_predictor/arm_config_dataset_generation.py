...
if __name__ == '__main__':
    ...
    q_robot_2_init = manip_env.robot_w.arm_rest_poses
    q_H_init = manip_env.human_rest_poses
    ...
    while True:
        ...
        for case in cases:
            for bias in biases:
                ...
                wiping_env.reset_robot(manip_env.robot_m, q_robot_init)
                env_pcd, arm_pcd, shoulder_pcd = manip_env.compute_env_pcd(robot=manip_env.robot_w, resolution=15)
                total_arm_pcd = np.vstack((arm_pcd, shoulder_pcd))

                # label pcd
                labeled_pcd_init = label_pcd(total_arm_pcd, targets)
                ...
                wiping_env.reset_robot(manip_env.robot_m, q_R_goal)
                env_pcd, arm_pcd, shoulder_pcd = manip_env.compute_env_pcd(robot=manip_env.robot_w, resolution=15)
                total_arm_pcd = np.vstack((arm_pcd, shoulder_pcd))
                ...
                # label pcd
                labeled_pcd_goal = label_pcd(total_arm_pcd, targets)
